...
$InstPath = Get-ChildItem -LiteralPath HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall |%{if($_.GetValue("DisplayName") -like "*ModusToolbox*" -and $_.GetValue("DisplayName") -notlike "*Edge*" -and $_.GetValue("DisplayName") -notlike "*Setup*" -and $_.GetValue("DisplayName") -notlike "*Pack*"){$_.GetValue("InstallLocation")}}|Select-Object -Last 1
if ( -not $InstPath ){ $InstPath = Get-ChildItem -LiteralPath HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall |%{if($_.GetValue("DisplayName") -like "*ModusToolbox*" -and $_.GetValue("DisplayName") -notlike "*Edge*" -and $_.GetValue("DisplayName") -notlike "*Setup*" -and $_.GetValue("DisplayName") -notlike "*Pack*"){$_.GetValue("InstallLocation")}}|Select-Object -Last 1 }
